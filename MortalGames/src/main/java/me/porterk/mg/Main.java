...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		tag = ChatColor.DARK_GRAY + "[" + ChatColor.DARK_RED + "MG"
		...
		Score time = obj.getScore(Bukkit.getOfflinePlayer(ChatColor.YELLOW
				+ "Time: "));
		time.setScore(0);
		Score round = obj.getScore(Bukkit.getOfflinePlayer(ChatColor.YELLOW
				+ "Wave: "));
		round.setScore(0);
		Score website = obj.getScore(Bukkit.getOfflinePlayer(ChatColor.RED + ""
				+ ChatColor.ITALIC + "mortal.gldesert.com"));
		...
		try {
			...
			Bukkit.getServer().getLogger()
					.log(Level.SEVERE, "Could not connect to MySQL server");
			;
		...
		try {
			getServer().getPluginManager().registerEvents(new MortalListener(),
					this);
		...
		try {
			...
			api.buildMapYML();
			Bukkit.getServer().getLogger()
					.log(Level.INFO, "Team bases constructed");
		} catch (Exception e) {
	...
	public boolean onCommand(CommandSender s, Command cmd, String label,
		...
		if (cmd.getName().equalsIgnoreCase("mg")) {
			...
			if (args[0].equalsIgnoreCase("admin")) {
				...
				if (chat.containsKey(p.getName())) {
					...
					if (!chat.get(p.getName()).equals("admin")) {
						...
						p.sendMessage(tag + ChatColor.DARK_RED + "Admin "
								+ ChatColor.WHITE + "chat has been "
			...
			if (args[0].equalsIgnoreCase("dev")) {
				...
				if (p.hasPermission("mortal.dev")) {
					...
					if (args[1].equalsIgnoreCase("start")) {
						...
						p.sendMessage(tag + ChatColor.GOLD
			...
			if (args[0].equals(null)) {
				...
				s.sendMessage(tag + ChatColor.GOLD
						+ "Do me a favor, learn the commands "
						+ ChatColor.DARK_RED + "[INVALID ARGUMENTS]");
		...
		Bukkit.getServer().getScheduler()
				.scheduleAsyncDelayedTask(plugin, new Runnable() {
					...
					public void run() {
						...
						createHelix(player);
						// Moved into createHelix to prevent ear rape.. Sounds
						// kinda cool but glitchy if both.
						player.playSound(player.getLocation(),
								Sound.ENDERMAN_TELEPORT, 1, 1.3F);
	...
	public static void createHelix(Player player) {
		...
		for (double y = 0; y <= 50; y += 0.05) {
			...
			Location loc = new Location(player.getLocation().getWorld(), player
					.getLocation().getX() + x, player.getLocation().getY() + y,
					player.getLocation().getZ() + z);
			loc.getWorld().playEffect(loc, Effect.WITCH_MAGIC, 0);
			// player.playSound(player.getLocation(),Sound.ENDERMAN_TELEPORT,1,
			// 1.3F);
		}
	}
}
